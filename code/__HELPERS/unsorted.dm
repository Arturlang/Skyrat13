...
//The variables should be apparent enough.
	...
	if(!location && !target)
		return
	var/obj/effect/temp_visual/animation = new /obj/effect/temp_visual(location)
	if(name)
		animation.name = name
	if(direction)
	...
	if(target && ismovable(target))
		...
		INVOKE_ASYNC(animation, /atom/movable/proc/orbit, AM, 0, FALSE, 0, 0, FALSE, TRUE)
	if(a_icon_state)
	...
	animation.timerid = (max(sleeptime, 15))
